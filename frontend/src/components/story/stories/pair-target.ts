...
/** Fresh row. 4+4 would reuse one seat; the real pair is 5 and 3. */
const PRACTICE = "[5,4,3]\n8";
...
function cells(nums: number[], paint: (index: number) => CellTone | null): GrokCell[] {
  ...
  return nums.map((value, index) => ({ value: String(value), tone: t[index], caption: String(index) }));
}
...
function picture(nums: number[], seen: Map<number, number>, paint: (index: number) => CellTone | null, extra?: Partial<GrokNotebookState>): GrokNotebookState {
  return {
    ...
    notebooks: [{ title: "notebook (value → seat)", entries: notes(seen) }],
    ...extra,
  };
}
  ...
  return [
    {
      ...
      state: picture(nums, new Map(), (index) => (index === at ? "edge" : null), { counter: { label: "partner", value: need } }),
    },
    {
      scene: "insight",
      caption: "Write each value and its seat in a notebook. Later boxes look the partner up in one glance.",
...
function solutionFrames(nums: number[], target: number, scene: SceneId = "solution", practice = false): Frame[] {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (trapHere && (practice || !askedTrap)) {
      ...
      frames.push({
        scene,
        caption: `${value} plus ${value} is the target. The notebook does not have a partner yet.`,
        codeLine: line(3),
    ...
    if (hit === undefined && trapHere) seen.set(value, i);
  }
  ...
  if (!practice) {
    ...
    frames.push({
      ...
      state: picture(nums, pair ? new Map([[nums[pair[0]], pair[0]]]) : new Map(), (index) =>
        pair && (index === pair[0] || index === pair[1]) ? "done" : "faded",
      ),
    });
  } else if (pair) {
    frames.push({
...
export const pairTargetStory: ProblemStory<GrokNotebookState> = {
  slugs: ["pair-target"],
  ...
  complexity: {
    slow: "O(n²)",
    time: "O(n)",
    timeWhy: "each value is stored and looked up once",
    space: "O(n)",
