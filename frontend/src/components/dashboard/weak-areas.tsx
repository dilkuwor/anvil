...
export function WeakAreasPanel({ areas }: { areas: WeakArea[] }) {
  ...
  return (
    <section className="rounded-xl border border-steel-800 bg-steel-900/60 p-5">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Your Weak Areas</h2>
      <ul className="mt-4 space-y-3">
        {areas.map((area) => (
          <li key={area.slug}>
            <div className="mb-1 flex justify-between text-sm">
              <span>{area.tag}</span>
              <span className="tabular-nums text-zinc-400">{Math.round(area.proficiency)}%</span>
            </div>
