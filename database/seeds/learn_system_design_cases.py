...
from database.seeds.sd_extra_cases import extra_cases
...
:::viz architecture {"title": "URL shortener: create a link, then redirect", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "lb", "label": "Load balancer", "kind": "edge", "col": 1, "row": 1}, {"id": "link", "label": "Link service", "kind": "service", "col": 2, "row": 1, "note": "stateless"}, {"id": "redis", "label": "Redis", "kind": "cache", "col": 3, "row": 0, "note": "code to long_url"}, {"id": "db", "label": "Postgres", "kind": "db", "col": 3, "row": 1, "note": "source of truth"}], "edges": [{"from": "client", "to": "lb"}, {"from": "lb", "to": "link"}, {"from": "link", "to": "redis", "label": "read first"}, {"from": "link", "to": "db"}], "steps": [{"title": "Create a link", "explain": "The client posts a long URL. The link service writes the new code to Postgres, then puts it in Redis right away because a fresh link is about to be clicked.", "interview": "Say that 40 writes per second is easy for one Postgres, and that you fill the cache on write because new links are clicked within seconds.", "path": ["client>lb", "lb>link", "link>db", "link>redis"], "kind": "invariant"}, {"title": "A redirect hits the cache", "explain": "Someone clicks the short link. The service looks up the code in Redis, finds it, and answers with a 301 in about a millisecond. Postgres is not touched.", "interview": "Say that reads are 300 times the writes, that popularity follows a power law, so the hit rate will be above 95 percent and this is the path that matters.", "path": ["client>lb", "lb>link", "link>redis"], "kind": "decision"}, {"title": "A cache miss", "explain": "An old or rare link is not in Redis. The service reads Postgres, fills the cache, and answers. This is the only read traffic the database sees.", "interview": "Say what Postgres must handle: a few hundred reads per second from misses, and what happens if Redis is cold after a restart.", "path": ["client>lb", "lb>link", "link>redis", "link>db"], "kind": "tradeoff", "hot": ["db"]}, {"title": "The whole path", "explain": "Client, balancer, stateless link service, Redis in front of Postgres. Writes go to the database and the cache; reads almost always stop at the cache.", "interview": "Close by naming what breaks first at ten times the load: Redis capacity for hot codes, then Postgres on cold misses, and say how you would shard each.", "path": ["client>lb", "lb>link", "link>redis", "link>db"], "kind": "result"}]}
...
:::viz architecture {"title": "Rate limiter: decide before doing any expensive work", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "gw", "label": "Gateway", "kind": "edge", "col": 1, "row": 1, "note": "verifies token locally"}, {"id": "redis", "label": "Redis", "kind": "cache", "col": 2, "row": 0, "note": "token bucket, Lua"}, {"id": "upstream", "label": "Upstream service", "kind": "service", "col": 3, "row": 1}], "edges": [{"from": "client", "to": "gw"}, {"from": "gw", "to": "redis", "label": "one round trip"}, {"from": "gw", "to": "upstream", "label": "allowed"}], "steps": [{"title": "A request arrives", "explain": "The gateway verifies the signed API key by itself, with no database call. Nothing expensive has happened yet.", "interview": "Say the limiter runs before body parsing, before any database lookup, and before any upstream call, because it protects those resources.", "path": ["client>gw"], "kind": "invariant"}, {"title": "Ask Redis for a token", "explain": "The gateway runs one Lua script against the key rl:{api_key}. The script refills the bucket, takes one token if there is one, and answers yes or no atomically.", "interview": "Say why it is one script and not two commands: read-then-write across 50 gateways would race, and the script makes the decision atomic.", "path": ["gw>redis"], "kind": "decision"}, {"title": "Allowed, or 429", "explain": "On yes, the gateway proxies to the upstream service. On no, it answers 429 with a Retry-After header and the upstream never sees the request.", "interview": "Say what you do if Redis is down: fail open and log it, because blocking every customer is worse than briefly over-admitting.", "path": ["gw>upstream"], "kind": "tradeoff", "hot": ["redis"]}, {"title": "The whole path", "explain": "Client, gateway, one Redis call, then the upstream. The limiter adds one sub-millisecond hop to every request and nothing else.", "interview": "Close with the global counting problem: 50 gateways share one Redis, and name local budgets with reconciliation as the optimisation if Redis becomes the bottleneck.", "path": ["client>gw", "gw>redis", "gw>upstream"], "kind": "result"}]}
...
:::viz architecture {"title": "Key-value store: one request, three replicas, a quorum", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "coord", "label": "Coordinator", "kind": "service", "col": 1, "row": 1, "note": "any node can be one"}, {"id": "r1", "label": "Replica 1", "kind": "db", "col": 2, "row": 0, "note": "owner on the ring"}, {"id": "r2", "label": "Replica 2", "kind": "db", "col": 2, "row": 1, "note": "next on the ring"}, {"id": "r3", "label": "Replica 3", "kind": "db", "col": 2, "row": 2, "note": "next after that"}], "edges": [{"from": "client", "to": "coord", "label": "put or get"}, {"from": "coord", "to": "r1"}, {"from": "coord", "to": "r2"}, {"from": "coord", "to": "r3"}], "steps": [{"title": "The request lands on any node", "explain": "The client sends put(k, v) to any node. That node hashes the key onto the ring and finds the three owners. There is no master.", "interview": "Say that consistent hashing means a node joining or leaving moves only its neighbours' keys, and that virtual nodes keep the load even.", "path": ["client>coord"], "kind": "invariant"}, {"title": "A write fans out, W = 2", "explain": "The coordinator sends the write to all three replicas with a version. It answers the client as soon as two confirm. The third can catch up later.", "interview": "Say the quorum rule out loud: with N = 3, W = 2 and R = 2, W + R is greater than N, so every read overlaps every write on at least one node.", "path": ["coord>r1", "coord>r2", "coord>r3"], "kind": "decision", "hot": ["r3"]}, {"title": "A read asks R = 2", "explain": "The coordinator asks two replicas and returns the newest version. If they disagree, it repairs the stale one in the background.", "interview": "Say what you give up with W = 1 and R = 1: faster answers, but a read can briefly see an old value, and name a use where that is fine.", "path": ["coord>r1", "coord>r2"], "kind": "tradeoff"}, {"title": "The whole path", "explain": "Client to any node, hash to three owners, fan out, wait for a quorum. Every node is a coordinator, so nothing is a single point of failure.", "interview": "Close by naming what happens when one replica is down: hinted handoff keeps writes flowing, and the replica catches up when it returns.", "path": ["client>coord", "coord>r1", "coord>r2", "coord>r3"], "kind": "result"}]}
...
:::viz architecture {"title": "News feed: the hybrid fan-out", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "api", "label": "Feed API", "kind": "service", "col": 1, "row": 1, "note": "stateless"}, {"id": "posts", "label": "Posts DB", "kind": "db", "col": 2, "row": 2, "note": "one row per post"}, {"id": "fanq", "label": "Fan-out queue", "kind": "queue", "col": 3, "row": 2}, {"id": "workers", "label": "Fan-out workers", "kind": "worker", "col": 4, "row": 2}, {"id": "celeb", "label": "Celebrity posts", "kind": "cache", "col": 4, "row": 1, "note": "one entry, millions of readers"}, {"id": "feeds", "label": "Feed lists", "kind": "cache", "col": 4, "row": 0, "note": "precomputed per user"}], "edges": [{"from": "client", "to": "api"}, {"from": "api", "to": "posts", "label": "store post"}, {"from": "api", "to": "fanq", "label": "normal users only"}, {"from": "fanq", "to": "workers"}, {"from": "workers", "to": "feeds", "label": "insert post id"}, {"from": "api", "to": "feeds", "label": "read list"}, {"from": "api", "to": "celeb", "label": "merge at read"}], "steps": [{"title": "A normal user posts", "explain": "The post is stored once, then a job goes on the queue. Workers insert the post id into each follower's feed list. This is fan-out on write.", "interview": "Say that most accounts have few followers, so fan-out on write is cheap for them and turns the feed read into one range read.", "path": ["client>api", "api>posts", "api>fanq", "fanq>workers", "workers>feeds"], "kind": "decision"}, {"title": "A celebrity posts", "explain": "Above the follower threshold there is no fan-out. The post is stored once and nothing else happens. Millions of writes are avoided.", "interview": "Say the threshold, say it is tunable, and say why: the crossover is where the write cost of fanning out exceeds the read cost of merging.", "path": ["client>api", "api>posts"], "kind": "tradeoff", "hot": ["fanq"]}, {"title": "A feed read merges both", "explain": "The API reads the user's precomputed list, then merges in recent posts from the few celebrities they follow. Those posts are heavily cached.", "interview": "Say the merge is cheap because a user follows only a handful of huge accounts, and one cache entry for a celebrity serves millions of readers.", "path": ["client>api", "api>feeds", "api>celeb"], "kind": "invariant"}, {"title": "The whole path", "explain": "Two write paths depending on follower count, and one read path that merges. This hybrid is the expected answer.", "interview": "Close by naming what fan-out on write gets wrong: unfollows and deletes leave stale ids in lists, and say how you filter them at read time.", "path": ["client>api", "api>posts", "api>fanq", "fanq>workers", "workers>feeds", "api>feeds", "api>celeb"], "kind": "result"}]}""",
            ),
            (
                "How It Works",
                """### The write path
...
:::viz architecture {"title": "Photo sharing: upload path and read path are separate", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "storage", "label": "Object storage", "kind": "storage", "col": 1, "row": 0, "note": "originals + renditions"}, {"id": "events", "label": "Event queue", "kind": "queue", "col": 2, "row": 0}, {"id": "workers", "label": "Processing workers", "kind": "worker", "col": 2, "row": 1, "note": "validate, strip EXIF, resize"}, {"id": "lb", "label": "Load balancer", "kind": "edge", "col": 1, "row": 2}, {"id": "api", "label": "API", "kind": "service", "col": 2, "row": 2, "note": "returns CDN URLs"}, {"id": "feed", "label": "Feed service", "kind": "service", "col": 3, "row": 1}, {"id": "metadb", "label": "Metadata DB", "kind": "db", "col": 3, "row": 2}], "edges": [{"from": "client", "to": "storage", "label": "pre-signed upload"}, {"from": "storage", "to": "events", "label": "event"}, {"from": "events", "to": "workers"}, {"from": "workers", "to": "storage", "label": "renditions"}, {"from": "workers", "to": "metadb", "label": "mark ready"}, {"from": "client", "to": "lb"}, {"from": "lb", "to": "api"}, {"from": "api", "to": "feed"}, {"from": "api", "to": "metadb"}], "steps": [{"title": "Upload goes straight to storage", "explain": "The client asks for a pre-signed URL and puts the original photo directly into object storage. The bytes never pass through your API.", "interview": "Say why the API only hands out URLs: it keeps the servers small and stateless, and object storage is built for this traffic.", "path": ["client>storage"], "kind": "invariant"}, {"title": "Workers make the renditions", "explain": "Storage emits an event. A worker validates the image, strips EXIF, writes thumbnail, grid, feed and full sizes back to storage, then marks the post ready.", "interview": "Say that stripping EXIF protects location privacy, and that renditions are written under deterministic keys so the CDN can find them.", "path": ["storage>events", "events>workers", "workers>storage", "workers>metadb"], "kind": "decision"}, {"title": "A feed read", "explain": "The client asks the API for its feed. The API talks to the feed service and the metadata DB and answers with JSON that holds CDN URLs for each image.", "interview": "Say that image bytes come from the CDN, not from your API, and that this is why the two flows are drawn as separate systems.", "path": ["client>lb", "lb>api", "api>feed", "api>metadb"], "kind": "tradeoff"}, {"title": "The whole picture", "explain": "An upload path of storage, events and workers, and a read path of API, feed service and metadata. They share only the storage bucket and the database.", "interview": "Close by saying which half scales how: workers scale with uploads, API servers scale with reads, and the CDN carries the bandwidth.", "path": ["client>storage", "storage>events", "events>workers", "workers>storage", "workers>metadb", "client>lb", "lb>api", "api>feed", "api>metadb"], "kind": "result"}]}
...
:::viz architecture {"title": "Video platform: upload, transcode, play", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "storage", "label": "Object storage", "kind": "storage", "col": 1, "row": 0, "note": "originals + segments"}, {"id": "queue", "label": "Job queue", "kind": "queue", "col": 2, "row": 0, "note": "one job per chunk"}, {"id": "workers", "label": "Transcoding workers", "kind": "worker", "col": 3, "row": 1, "note": "parallel, interruptible"}, {"id": "api", "label": "API", "kind": "service", "col": 1, "row": 2, "note": "JSON and URLs only"}, {"id": "cdn", "label": "CDN", "kind": "edge", "col": 2, "row": 2, "note": "serves segments"}], "edges": [{"from": "client", "to": "storage", "label": "pre-signed multipart"}, {"from": "storage", "to": "queue", "label": "upload complete"}, {"from": "queue", "to": "workers"}, {"from": "workers", "to": "storage", "label": "segments + manifest"}, {"from": "client", "to": "api", "label": "metadata, manifest URL"}, {"from": "client", "to": "cdn", "label": "segments"}, {"from": "cdn", "to": "storage", "label": "origin pull", "dashed": true}], "steps": [{"title": "Upload goes straight to storage", "explain": "The client uploads the original in parts using pre-signed URLs. When the upload completes, storage emits an event and a job goes on the queue.", "interview": "Say that video bytes never pass through your application tier, and that multipart upload makes a lost connection resumable.", "path": ["client>storage", "storage>queue"], "kind": "invariant"}, {"title": "Transcode by chunk", "explain": "A coordinator splits the video into small chunks and one job per chunk per rendition. Workers run them in parallel and write segments and a manifest back to storage.", "interview": "Say that chunking turns a 20-minute serial transcode into a 2-minute parallel one, and that small jobs can run on cheap interruptible machines.", "path": ["queue>workers", "workers>storage"], "kind": "decision"}, {"title": "Playback", "explain": "The client asks the API for the video's metadata and a signed manifest URL. Then it fetches segments from the CDN, which pulls from storage on a miss.", "interview": "Say that the player picks a rendition per segment based on bandwidth, and that the CDN hit rate is what keeps origin traffic small.", "path": ["client>api", "client>cdn", "cdn>storage"], "kind": "tradeoff"}, {"title": "The whole picture", "explain": "Upload to storage, a queue and workers that transcode, and a playback path where the API serves JSON and the CDN serves bytes.", "interview": "Close by naming the state machine the client sees: uploading, processing, ready or failed, and say why exposing it beats a spinner.", "path": ["client>storage", "storage>queue", "queue>workers", "workers>storage", "client>api", "client>cdn", "cdn>storage"], "kind": "result"}]}
...
:::viz architecture {"title": "Streaming service: a small request system, a huge static one", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "api", "label": "API", "kind": "service", "col": 1, "row": 0, "note": "entitlement, session"}, {"id": "catdb", "label": "Catalogue DB", "kind": "db", "col": 2, "row": 0}, {"id": "cdn", "label": "Edge caches / CDN", "kind": "edge", "col": 2, "row": 1, "note": "segments, near the user"}, {"id": "origin", "label": "Origin storage", "kind": "storage", "col": 3, "row": 1, "note": "every rendition"}, {"id": "encoder", "label": "Encoding pipeline", "kind": "worker", "col": 4, "row": 1, "note": "offline, per title"}], "edges": [{"from": "client", "to": "api", "label": "play: signed manifest"}, {"from": "api", "to": "catdb"}, {"from": "client", "to": "cdn", "label": "segments"}, {"from": "origin", "to": "cdn", "label": "pushed ahead of demand"}, {"from": "encoder", "to": "origin", "label": "encoded renditions"}], "steps": [{"title": "A play request", "explain": "The client asks to play a title. The API checks the profile is entitled, reads the catalogue, and returns a signed manifest that points at the edge.", "interview": "Say that this half is a modest web service: a few thousand requests per second, small JSON answers, and a catalogue that rarely changes.", "path": ["client>api", "api>catdb"], "kind": "invariant"}, {"title": "Prepared in advance", "explain": "Long before anyone presses play, each title is encoded once, exhaustively, and pushed to edge locations during quiet hours.", "interview": "Say that a fixed catalogue lets you spend heavily on per-title encoding, because the cost is paid once and served millions of times.", "path": ["encoder>origin", "origin>cdn"], "kind": "decision"}, {"title": "Segments from the edge", "explain": "The player fetches segments from the nearest edge cache. Almost nothing goes back to origin, so your own network carries very little.", "interview": "Say that popularity is predictable, so you pre-position the top titles and the edge hit rate is close to 100 percent.", "path": ["client>cdn"], "kind": "tradeoff", "hot": ["cdn"]}, {"title": "The whole picture", "explain": "One small request-serving system and one enormous, mostly static content distribution system. The two barely touch.", "interview": "Close by naming what a new release does: a spike on the request side and, if pre-positioned well, no spike at origin at all.", "path": ["client>api", "api>catdb", "encoder>origin", "origin>cdn", "client>cdn"], "kind": "result"}]}
...
:::viz architecture {"title": "Chat: a message goes in, and out to every recipient device", "nodes": [{"id": "client", "label": "Sender", "kind": "client", "col": 0, "row": 1}, {"id": "edge", "label": "Edge", "kind": "edge", "col": 1, "row": 1}, {"id": "gw", "label": "Gateway", "kind": "service", "col": 2, "row": 1, "note": "holds sockets only"}, {"id": "gw2", "label": "Other gateways", "kind": "service", "col": 2, "row": 0, "note": "recipient sockets"}, {"id": "msg", "label": "Message service", "kind": "service", "col": 3, "row": 1, "note": "stateless"}, {"id": "pubsub", "label": "Pub/Sub", "kind": "queue", "col": 3, "row": 0}, {"id": "push", "label": "Push service", "kind": "external", "col": 4, "row": 0, "note": "offline devices"}, {"id": "store", "label": "Message store", "kind": "db", "col": 4, "row": 1, "note": "by conversation"}, {"id": "registry", "label": "Registry", "kind": "cache", "col": 4, "row": 2, "note": "user to gateway"}], "edges": [{"from": "client", "to": "edge"}, {"from": "edge", "to": "gw"}, {"from": "gw", "to": "msg"}, {"from": "msg", "to": "store", "label": "persist first"}, {"from": "msg", "to": "registry", "label": "who is online"}, {"from": "msg", "to": "pubsub"}, {"from": "pubsub", "to": "gw2", "label": "deliver"}, {"from": "msg", "to": "push", "label": "offline"}], "steps": [{"title": "A message comes in", "explain": "The sender's socket lives on one gateway. The gateway forwards the message to a stateless message service, which writes it to the store first.", "interview": "Say that the gateway holds connections and nothing else, so a gateway crash loses sockets, not messages, and clients simply reconnect.", "path": ["client>edge", "edge>gw", "gw>msg", "msg>store"], "kind": "invariant"}, {"title": "Deliver to online devices", "explain": "The service asks the registry which gateways hold the recipient's devices, then publishes. Those gateways push the message down their sockets.", "interview": "Say that the store write comes before the publish, so an ack to the sender always means the message is durable.", "path": ["msg>registry", "msg>pubsub", "pubsub>gw2"], "kind": "decision"}, {"title": "Offline devices get a push", "explain": "A device with no socket gets a push notification instead. When it reconnects it syncs everything after its last seen sequence number.", "interview": "Say the registry is a hot component with 100 million entries and constant updates, and that it belongs in a sharded in-memory store.", "path": ["msg>push"], "kind": "tradeoff", "hot": ["registry"]}, {"title": "The whole path", "explain": "Edge, gateways that only hold sockets, a stateless message service, a store, pub/sub back out to gateways, and push for offline devices.", "interview": "Close by naming how ordering works: a per-conversation sequence number, so every device shows the same order however messages arrive.", "path": ["client>edge", "edge>gw", "gw>msg", "msg>store", "msg>registry", "msg>pubsub", "pubsub>gw2", "msg>push"], "kind": "result"}]}
...
:::viz architecture {"title": "Notification system: policy in the router, delivery in dumb workers", "nodes": [{"id": "producers", "label": "Producers", "kind": "client", "col": 0, "row": 0, "note": "other services"}, {"id": "api", "label": "Notification API", "kind": "service", "col": 1, "row": 0, "note": "accepts, dedupes key"}, {"id": "queue", "label": "Queue", "kind": "queue", "col": 2, "row": 0, "note": "absorbs bursts"}, {"id": "router", "label": "Router", "kind": "service", "col": 3, "row": 0, "note": "prefs, limits, render"}, {"id": "prefs", "label": "Preferences", "kind": "db", "col": 3, "row": 1, "note": "suppressions, quiet hours"}, {"id": "chq", "label": "Per-channel queues", "kind": "queue", "col": 4, "row": 0}, {"id": "workers", "label": "Channel workers", "kind": "worker", "col": 4, "row": 1, "note": "call, record, retry"}, {"id": "providers", "label": "Providers", "kind": "external", "col": 4, "row": 2, "note": "APNs, email, SMS"}, {"id": "deliveries", "label": "Deliveries", "kind": "db", "col": 3, "row": 2, "note": "did they get it?"}], "edges": [{"from": "producers", "to": "api"}, {"from": "api", "to": "queue"}, {"from": "queue", "to": "router"}, {"from": "router", "to": "prefs"}, {"from": "router", "to": "chq"}, {"from": "chq", "to": "workers"}, {"from": "workers", "to": "providers"}, {"from": "workers", "to": "deliveries", "label": "outcome"}], "steps": [{"title": "Accept and queue", "explain": "A producer sends an event with an event_key. The API stores it and puts it on the queue. The producer gets a 202 in milliseconds.", "interview": "Say that a unique constraint on event_key makes a producer's retry a no-op, and that this is the cheapest line of defence against duplicates.", "path": ["producers>api", "api>queue"], "kind": "invariant"}, {"title": "The router decides", "explain": "The router checks suppressions, preferences, quiet hours and per-user rate limits, picks the channels, renders the template, and enqueues per channel.", "interview": "Say that quiet hours must not apply to transactional messages, and that the per-user rate limit is what stops one buggy producer from spamming.", "path": ["queue>router", "router>prefs", "router>chq"], "kind": "decision"}, {"title": "Workers deliver", "explain": "A worker takes one rendered message, calls the provider, records the outcome, and retries on failure. It knows nothing about policy.", "interview": "Say why the two layers are separate: policy changes constantly, delivery must be simple and fast, and a provider outage only backs up its own queue.", "path": ["chq>workers", "workers>providers", "workers>deliveries"], "kind": "tradeoff", "hot": ["providers"]}, {"title": "The whole path", "explain": "Producers, API, queue, router, per-channel queues, workers, providers. The router is the interesting box; everything else is plumbing.", "interview": "Close by naming how you answer 'did they get it?': the deliveries table, updated by provider callbacks, one row per channel attempt.", "path": ["producers>api", "api>queue", "queue>router", "router>prefs", "router>chq", "chq>workers", "workers>providers", "workers>deliveries"], "kind": "result"}]}
...
:::viz architecture {"title": "Ride sharing: an ephemeral location system beside a transactional trip system", "nodes": [{"id": "driver", "label": "Driver app", "kind": "client", "col": 0, "row": 0}, {"id": "location", "label": "Location service", "kind": "cache", "col": 1, "row": 0, "note": "in-memory, geo-indexed"}, {"id": "realtime", "label": "Real-time channel", "kind": "edge", "col": 1, "row": 1, "note": "sockets to both apps"}, {"id": "matching", "label": "Matching service", "kind": "service", "col": 2, "row": 1}, {"id": "rider", "label": "Rider app", "kind": "client", "col": 0, "row": 2}, {"id": "ride", "label": "Ride service", "kind": "service", "col": 1, "row": 2}, {"id": "trip", "label": "Trip store", "kind": "db", "col": 2, "row": 2, "note": "relational"}, {"id": "payment", "label": "Payment service", "kind": "service", "col": 3, "row": 2}], "edges": [{"from": "driver", "to": "location", "label": "position every few seconds"}, {"from": "rider", "to": "ride", "label": "request ride"}, {"from": "ride", "to": "matching"}, {"from": "matching", "to": "location", "label": "nearby drivers"}, {"from": "matching", "to": "realtime", "label": "offer"}, {"from": "realtime", "to": "driver"}, {"from": "realtime", "to": "rider", "label": "trip state"}, {"from": "ride", "to": "trip"}, {"from": "trip", "to": "payment", "label": "on completion"}], "steps": [{"title": "Drivers report where they are", "explain": "Every few seconds each driver app sends its position. The location service keeps it in memory, indexed by geo cell, and forgets it after 30 seconds of silence.", "interview": "Say this is not a database: millions of writes per minute, tiny values, and staleness after 30 seconds is fine because the driver has moved anyway.", "path": ["driver>location"], "kind": "invariant"}, {"title": "A rider asks, a driver is offered", "explain": "The ride service creates the request and asks matching for a driver. Matching queries nearby drivers, picks one, and sends the offer over the real-time channel.", "interview": "Say how you stop double-booking: one conditional update on the driver's state row, so two matches for the same driver cannot both win.", "path": ["rider>ride", "ride>matching", "matching>location", "matching>realtime", "realtime>driver"], "kind": "decision"}, {"title": "The trip is recorded and paid", "explain": "The trip row moves through its states in the relational store. Both apps see updates over the real-time channel. On completion, payment runs.", "interview": "Say that the trip store is low-rate and transactional, the opposite of the location service, and that drawing them as separate systems is most of the design.", "path": ["ride>trip", "trip>payment", "realtime>rider"], "kind": "tradeoff"}, {"title": "The whole picture", "explain": "A high-rate in-memory location system, a matching service that reads it, and a low-rate transactional trip system with payments behind it.", "interview": "Close by naming what happens if the location service loses everything: drivers re-report within seconds, and nothing about trips is lost.", "path": ["driver>location", "rider>ride", "ride>matching", "matching>location", "matching>realtime", "realtime>driver", "realtime>rider", "ride>trip", "trip>payment"], "kind": "result"}]}
...
:::viz architecture {"title": "File sync: metadata on one path, bytes on another", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "meta", "label": "Metadata service", "kind": "service", "col": 1, "row": 0}, {"id": "metadb", "label": "Metadata DB", "kind": "db", "col": 2, "row": 0, "note": "sharded by user"}, {"id": "changelog", "label": "Change log", "kind": "queue", "col": 2, "row": 1, "note": "per-user, ordered"}, {"id": "notify", "label": "Notification channel", "kind": "edge", "col": 3, "row": 1, "note": "long-poll or socket"}, {"id": "devices", "label": "Other devices", "kind": "client", "col": 4, "row": 1}, {"id": "chunk", "label": "Chunk service", "kind": "service", "col": 1, "row": 2, "note": "hash check"}, {"id": "storage", "label": "Object storage", "kind": "storage", "col": 2, "row": 2, "note": "content-addressed"}], "edges": [{"from": "client", "to": "meta", "label": "commit version"}, {"from": "meta", "to": "metadb"}, {"from": "meta", "to": "changelog"}, {"from": "changelog", "to": "notify"}, {"from": "notify", "to": "devices", "label": "something changed"}, {"from": "client", "to": "chunk", "label": "which chunks are new?"}, {"from": "chunk", "to": "storage", "label": "direct transfer"}], "steps": [{"title": "Only new chunks are uploaded", "explain": "The client splits the file into chunks and sends their hashes. The chunk service says which ones storage already has, and the client uploads only those, directly to storage.", "interview": "Say that a one-line edit to a large file uploads one chunk, not the file, and that identical chunks across users are stored once.", "path": ["client>chunk", "chunk>storage"], "kind": "decision"}, {"title": "Commit the metadata", "explain": "Once the chunks are in, the client commits a new file version: the chunk list, the size, and the parent version. The metadata DB is sharded by user.", "interview": "Say that the commit is the atomic step: a version either points at all its chunks or does not exist, so a crash mid-upload leaves nothing broken.", "path": ["client>meta", "meta>metadb"], "kind": "invariant"}, {"title": "Other devices find out", "explain": "The commit appends to the user's change log. The notification channel wakes the user's other devices, which fetch the changes after their cursor.", "interview": "Say that the metadata path is the one under load, and that a device syncs by asking for everything after its last seen change id.", "path": ["meta>changelog", "changelog>notify", "notify>devices"], "kind": "tradeoff"}, {"title": "The whole picture", "explain": "Bytes go client to storage through a hash check. Metadata goes to a sharded database, then out through a change log to every other device.", "interview": "Close by naming how conflicts are handled: the second commit with the same parent version loses and becomes a conflicted copy.", "path": ["client>chunk", "chunk>storage", "client>meta", "meta>metadb", "meta>changelog", "changelog>notify", "notify>devices"], "kind": "result"}]}
...
:::viz architecture {"title": "Autocomplete: an offline build, an in-memory read", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "edge", "label": "Edge cache", "kind": "edge", "col": 1, "row": 1, "note": "short prefixes"}, {"id": "suggest", "label": "Suggest service", "kind": "service", "col": 2, "row": 1, "note": "in-memory trie"}, {"id": "logs", "label": "Query logs", "kind": "storage", "col": 1, "row": 0}, {"id": "agg", "label": "Aggregate, filter, score", "kind": "worker", "col": 2, "row": 0, "note": "minutes to hours"}, {"id": "build", "label": "Trie build", "kind": "worker", "col": 3, "row": 0}, {"id": "dist", "label": "Distribution", "kind": "storage", "col": 3, "row": 1, "note": "trie snapshot"}], "edges": [{"from": "client", "to": "edge", "label": "each keystroke"}, {"from": "edge", "to": "suggest", "label": "on miss"}, {"from": "logs", "to": "agg"}, {"from": "agg", "to": "build"}, {"from": "build", "to": "dist"}, {"from": "dist", "to": "suggest", "label": "load new snapshot"}], "steps": [{"title": "A keystroke hits the edge", "explain": "The client sends the prefix on every keystroke. Short prefixes are the same for everyone, so the edge cache answers most of them without any service call.", "interview": "Say that the top suggestions for 'a' or 'th' change slowly, so a short TTL at the edge removes most of the load before it reaches you.", "path": ["client>edge"], "kind": "invariant"}, {"title": "A miss walks the trie", "explain": "The suggest service walks an in-memory trie to the prefix node and returns the precomputed top suggestions stored there. No database, no scoring, microseconds.", "interview": "Say that each trie node stores its top-k already, so a lookup is a walk of the prefix length and nothing more.", "path": ["edge>suggest"], "kind": "decision"}, {"title": "The offline build", "explain": "Query logs are aggregated, filtered and scored, then built into a new trie. The snapshot is distributed and each serving node swaps it in.", "interview": "Say that the build runs every few minutes to hours, and that the swap is atomic per node, so a bad build can be rolled back by loading the previous snapshot.", "path": ["logs>agg", "agg>build", "build>dist", "dist>suggest"], "kind": "tradeoff"}, {"title": "The whole picture", "explain": "Two paths that never meet at request time: an offline pipeline that builds the trie, and an online path that only reads memory.", "interview": "Close by saying the serving nodes never read a database, never call another service, and never compute a score.", "path": ["client>edge", "edge>suggest", "logs>agg", "agg>build", "build>dist", "dist>suggest"], "kind": "result"}]}
...
:::viz architecture {"title": "Web crawler: the loop through the frontier", "nodes": [{"id": "seeds", "label": "Scheduler / seeds", "kind": "client", "col": 0, "row": 0}, {"id": "frontier", "label": "Frontier", "kind": "queue", "col": 1, "row": 0, "note": "per-host queues"}, {"id": "fetchers", "label": "Fetcher pool", "kind": "worker", "col": 2, "row": 1, "note": "stateless"}, {"id": "parser", "label": "Parser", "kind": "service", "col": 3, "row": 1}, {"id": "filter", "label": "URL filter", "kind": "service", "col": 4, "row": 0, "note": "seen set, robots"}, {"id": "robots", "label": "Robots cache", "kind": "cache", "col": 1, "row": 2, "note": "per host"}, {"id": "pages", "label": "Page store", "kind": "storage", "col": 2, "row": 2, "note": "raw content"}, {"id": "dedup", "label": "Content dedup", "kind": "service", "col": 3, "row": 2, "note": "hash, SimHash"}], "edges": [{"from": "seeds", "to": "frontier"}, {"from": "frontier", "to": "fetchers", "label": "one URL per host at a time"}, {"from": "fetchers", "to": "robots"}, {"from": "fetchers", "to": "pages", "label": "raw page"}, {"from": "fetchers", "to": "parser"}, {"from": "parser", "to": "dedup"}, {"from": "parser", "to": "filter", "label": "new links"}, {"from": "filter", "to": "frontier", "label": "unseen URLs"}], "steps": [{"title": "Pick the next URL, politely", "explain": "The frontier hands a fetcher one URL, chosen by priority, and never two URLs for the same host at once. The fetcher checks the host's robots rules first.", "interview": "Say that politeness is enforced by the frontier's per-host queues, not by the fetchers, so adding fetchers cannot overload a site.", "path": ["seeds>frontier", "frontier>fetchers", "fetchers>robots"], "kind": "invariant"}, {"title": "Fetch, store, parse", "explain": "The fetcher downloads the page and stores the raw content. The parser extracts links and hashes the content so a copy of a known page is dropped.", "interview": "Say the fetcher pool is the part you scale horizontally, and that most fetcher time is spent waiting on the network, so run many per machine.", "path": ["fetchers>pages", "fetchers>parser", "parser>dedup"], "kind": "decision"}, {"title": "New links go back in", "explain": "The URL filter drops links already seen and links robots forbids. What is left goes back into the frontier, and the loop continues.", "interview": "Say the seen set is billions of URLs, so it is a Bloom filter or a sharded store, and name what a false positive costs: a page missed, not a crash.", "path": ["parser>filter", "filter>frontier"], "kind": "tradeoff", "hot": ["frontier"]}, {"title": "The whole loop", "explain": "Frontier, fetchers, parser, filter, frontier again. The fetchers are stateless; the frontier is the stateful part worth a deep dive.", "interview": "Close by naming the trap: a crawler trap or an infinite calendar, and say how per-host limits and depth caps keep it from eating the budget.", "path": ["seeds>frontier", "frontier>fetchers", "fetchers>robots", "fetchers>pages", "fetchers>parser", "parser>dedup", "parser>filter", "filter>frontier"], "kind": "result"}]}
...
:::viz architecture {"title": "Job scheduler: due jobs become runs, runs become work", "nodes": [{"id": "client", "label": "Job owners", "kind": "client", "col": 0, "row": 1}, {"id": "api", "label": "Jobs API", "kind": "service", "col": 1, "row": 1}, {"id": "jobsdb", "label": "Jobs table", "kind": "db", "col": 1, "row": 0, "note": "next_run_at"}, {"id": "scheduler", "label": "Scheduler", "kind": "service", "col": 2, "row": 1, "note": "loop, every second"}, {"id": "runs", "label": "Run records", "kind": "db", "col": 3, "row": 0, "note": "unique per job, time, attempt"}, {"id": "queue", "label": "Dispatch queue", "kind": "queue", "col": 3, "row": 1}, {"id": "workers", "label": "Workers", "kind": "worker", "col": 4, "row": 1, "note": "lease + heartbeat"}, {"id": "reaper", "label": "Reaper", "kind": "worker", "col": 4, "row": 0, "note": "expired leases"}], "edges": [{"from": "client", "to": "api", "label": "create job"}, {"from": "api", "to": "jobsdb"}, {"from": "scheduler", "to": "jobsdb", "label": "due jobs"}, {"from": "scheduler", "to": "runs", "label": "insert run"}, {"from": "scheduler", "to": "queue"}, {"from": "queue", "to": "workers"}, {"from": "workers", "to": "runs", "label": "completion callback"}, {"from": "reaper", "to": "runs", "label": "find expired"}, {"from": "reaper", "to": "queue", "label": "retry"}], "steps": [{"title": "A job is created", "explain": "The owner registers a job with a cron expression. The API stores it with its first next_run_at.", "interview": "Say that a job and a run are different things: the job is the definition, and a run is one execution with its own state.", "path": ["client>api", "api>jobsdb"], "kind": "invariant"}, {"title": "The scheduler ticks", "explain": "Every second the scheduler finds jobs whose next_run_at is due, inserts one run row each, advances next_run_at, and pushes the runs onto the queue.", "interview": "Say that the unique constraint on (job_id, scheduled_for, attempt) is what makes two schedulers safe: the second insert fails and nothing runs twice.", "path": ["scheduler>jobsdb", "scheduler>runs", "scheduler>queue"], "kind": "decision"}, {"title": "A worker takes a lease", "explain": "A worker pulls a run, holds a lease, heartbeats while working, and reports the outcome on the run record when done.", "interview": "Say that at-least-once is the guarantee, so the job itself must be idempotent, and name the run id as the key it should use.", "path": ["queue>workers", "workers>runs"], "kind": "tradeoff"}, {"title": "The reaper catches the dead", "explain": "If a worker dies, its heartbeat stops and the lease expires. The reaper finds those runs and re-queues them as the next attempt.", "interview": "Say the interesting behaviour is the interaction of the uniqueness constraint and lease expiry: a slow worker and a retry can both run, so the lease must be longer than the job.", "path": ["reaper>runs", "reaper>queue"], "kind": "tradeoff", "hot": ["workers"]}, {"title": "The whole path", "explain": "Jobs table, a scheduler that turns due jobs into runs, a queue, workers with leases, and a reaper for the runs that never report back.", "interview": "Close by naming what happens if the scheduler is down for ten minutes: it catches up on restart, and you must decide whether missed runs fire once or all ten times.", "path": ["client>api", "api>jobsdb", "scheduler>jobsdb", "scheduler>runs", "scheduler>queue", "queue>workers", "workers>runs", "reaper>runs", "reaper>queue"], "kind": "result"}]}
...
:::viz architecture {"title": "Payments: authorise, hear back, reconcile", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "api", "label": "Payment API", "kind": "service", "col": 1, "row": 1, "note": "idempotency key"}, {"id": "paysvc", "label": "Payment service", "kind": "service", "col": 2, "row": 1, "note": "state machine"}, {"id": "adapter", "label": "Provider adapter", "kind": "service", "col": 3, "row": 1}, {"id": "provider", "label": "Provider", "kind": "external", "col": 4, "row": 1, "note": "card network"}, {"id": "webhook", "label": "Webhook receiver", "kind": "edge", "col": 4, "row": 0, "note": "queue behind it"}, {"id": "processor", "label": "Event processor", "kind": "worker", "col": 3, "row": 0}, {"id": "ledger", "label": "Payments + ledger", "kind": "db", "col": 2, "row": 2, "note": "same transaction"}, {"id": "recon", "label": "Reconciliation", "kind": "worker", "col": 4, "row": 2, "note": "daily settlement files"}], "edges": [{"from": "client", "to": "api"}, {"from": "api", "to": "paysvc"}, {"from": "paysvc", "to": "ledger"}, {"from": "paysvc", "to": "adapter"}, {"from": "adapter", "to": "provider", "label": "authorise"}, {"from": "provider", "to": "webhook", "label": "outcome"}, {"from": "webhook", "to": "processor"}, {"from": "processor", "to": "paysvc", "label": "update state"}, {"from": "recon", "to": "provider", "label": "settlement files", "dashed": true}, {"from": "recon", "to": "ledger", "label": "compare"}], "steps": [{"title": "Authorise", "explain": "The API stores the idempotency key, the service writes the payment as pending and a ledger entry in one transaction, then calls the provider through the adapter.", "interview": "Say that the duplicate request returns the stored response, and that the pending row is written before the provider call so a crash cannot lose a charge.", "path": ["client>api", "api>paysvc", "paysvc>ledger", "paysvc>adapter", "adapter>provider"], "kind": "invariant"}, {"title": "The provider calls back", "explain": "The final outcome arrives as a webhook. It is queued, then the event processor moves the payment to captured or failed and appends the ledger entry.", "interview": "Say that webhooks arrive out of order and more than once, so the processor applies them by event id and ignores anything already seen.", "path": ["provider>webhook", "webhook>processor", "processor>paysvc", "paysvc>ledger"], "kind": "decision"}, {"title": "Reconcile against the provider", "explain": "Each day the reconciliation job pulls the provider's settlement file and compares every line to the ledger. Mismatches go to an exceptions queue for a human.", "interview": "Say that reconciliation is a first-class component, not an afterthought, because timeouts leave payments whose true state only the provider knows.", "path": ["recon>provider", "recon>ledger"], "kind": "tradeoff", "hot": ["ledger"]}, {"title": "The whole picture", "explain": "A synchronous authorise path, an asynchronous webhook path, and a daily reconciliation path, all meeting at the ledger.", "interview": "Close by naming the ledger's rule: append-only, double-entry, and never updated in place, so every balance can be recomputed from history.", "path": ["client>api", "api>paysvc", "paysvc>ledger", "paysvc>adapter", "adapter>provider", "provider>webhook", "webhook>processor", "processor>paysvc", "recon>provider", "recon>ledger"], "kind": "result"}]}
...
:::viz architecture {"title": "Ticket booking: browse approximately, buy exactly", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "cdn", "label": "CDN", "kind": "edge", "col": 1, "row": 0}, {"id": "seatcache", "label": "Seat map cache", "kind": "cache", "col": 2, "row": 0, "note": "Redis, approximate"}, {"id": "waiting", "label": "Waiting room", "kind": "edge", "col": 1, "row": 2, "note": "admits a trickle"}, {"id": "booking", "label": "Booking service", "kind": "service", "col": 2, "row": 2}, {"id": "seats", "label": "Seats table", "kind": "db", "col": 3, "row": 1, "note": "conditional update"}, {"id": "payment", "label": "Payment", "kind": "external", "col": 3, "row": 2}, {"id": "reaper", "label": "Reaper", "kind": "worker", "col": 4, "row": 1, "note": "expired holds"}], "edges": [{"from": "client", "to": "cdn", "label": "browse"}, {"from": "cdn", "to": "seatcache"}, {"from": "booking", "to": "seatcache", "label": "async updates", "dashed": true}, {"from": "client", "to": "waiting", "label": "buy"}, {"from": "waiting", "to": "booking"}, {"from": "booking", "to": "seats", "label": "hold, then confirm"}, {"from": "booking", "to": "payment"}, {"from": "reaper", "to": "seats", "label": "release"}], "steps": [{"title": "Browsing never touches the database", "explain": "200,000 people a second look at the seat map. They read a cached copy from the CDN and Redis that may be a few seconds stale.", "interview": "Say that a stale seat map is fine because the hold step is exact, and that this separation is the whole design.", "path": ["client>cdn", "cdn>seatcache"], "kind": "invariant"}, {"title": "A hold is one conditional update", "explain": "A buyer passes the waiting room and asks to hold a seat. The booking service runs one update: set held where seat is free. Zero rows changed means someone else got it.", "interview": "Say the exact SQL out loud, with the WHERE clause on the seat's current state, and say why this needs no lock and no transaction across rows.", "path": ["client>waiting", "waiting>booking", "booking>seats"], "kind": "decision", "hot": ["seats"]}, {"title": "Pay, confirm, and tell the map", "explain": "With the hold in place, payment runs. On success the hold becomes a booking with another conditional update, and the seat map cache is updated asynchronously.", "interview": "Say that the hold has a TTL, so an abandoned checkout frees the seat, and that the reaper is what enforces it.", "path": ["booking>payment", "booking>seats", "booking>seatcache"], "kind": "tradeoff"}, {"title": "The reaper frees the abandoned", "explain": "Every few seconds the reaper releases holds whose time is up. Those seats become free again and the map shows them shortly after.", "interview": "Say that a release is also conditional: only if the hold is still the same one, so it cannot free a seat that was just confirmed.", "path": ["reaper>seats"], "kind": "invariant"}, {"title": "The whole picture", "explain": "A high-volume browse path through caches, and a low-volume buy path through a waiting room to the one table that holds the truth.", "interview": "Close by naming the number: a few thousand hold attempts per second is the only load on the seats table, and it handles that easily.", "path": ["client>cdn", "cdn>seatcache", "client>waiting", "waiting>booking", "booking>seats", "booking>payment", "booking>seatcache", "reaper>seats"], "kind": "result"}]}
...
:::viz architecture {"title": "API gateway: a data plane that never waits on the control plane", "nodes": [{"id": "client", "label": "Client", "kind": "client", "col": 0, "row": 1}, {"id": "gw", "label": "Gateway node", "kind": "edge", "col": 2, "row": 1, "note": "config in memory"}, {"id": "keys", "label": "Public keys", "kind": "cache", "col": 2, "row": 0, "note": "cached, verify locally"}, {"id": "rl", "label": "Rate-limit budget", "kind": "cache", "col": 3, "row": 0, "note": "local bucket, shared budget"}, {"id": "upstream", "label": "Upstream service", "kind": "service", "col": 3, "row": 1}, {"id": "authoring", "label": "Config authoring", "kind": "client", "col": 0, "row": 2, "note": "control plane"}, {"id": "validate", "label": "Validation", "kind": "service", "col": 1, "row": 2}, {"id": "store", "label": "Versioned store", "kind": "db", "col": 2, "row": 2, "note": "immutable snapshots"}, {"id": "dist", "label": "Distribution", "kind": "worker", "col": 3, "row": 2, "note": "pushes to every node"}], "edges": [{"from": "client", "to": "gw"}, {"from": "gw", "to": "keys", "label": "auth"}, {"from": "gw", "to": "rl", "label": "limit"}, {"from": "gw", "to": "upstream", "label": "route"}, {"from": "authoring", "to": "validate"}, {"from": "validate", "to": "store"}, {"from": "store", "to": "dist"}, {"from": "dist", "to": "gw", "label": "new config version"}], "steps": [{"title": "A request is checked locally", "explain": "The gateway matches the route from memory, verifies the token with cached public keys, and takes a rate-limit token from a local bucket. No network calls yet.", "interview": "Say the ordering principle: cheap rejections first, so an unknown route or an oversized body is refused before any authentication work.", "path": ["client>gw", "gw>keys", "gw>rl"], "kind": "invariant"}, {"title": "Route upstream", "explain": "The gateway strips any client-supplied identity headers, adds trusted ones, picks a healthy upstream instance, and applies the timeout.", "interview": "Say the security detail out loud: the gateway must strip inbound headers it uses for trusted identity, or a client can claim to be any tenant.", "path": ["gw>upstream"], "kind": "decision"}, {"title": "A config change arrives", "explain": "Someone publishes a new route. It is validated, saved as an immutable numbered version, and pushed to every node, which swaps it in atomically.", "interview": "Say that rollback is republishing a previous version number, and that this takes seconds and is the most valuable operational property here.", "path": ["authoring>validate", "validate>store", "store>dist", "dist>gw"], "kind": "tradeoff", "hot": ["store"]}, {"title": "The whole picture", "explain": "Two planes. The data plane serves 500,000 requests a second from memory. The control plane can be down and traffic continues on the last good config.", "interview": "Close by saying what a live config lookup per request would mean: the config store becomes a single point of failure for the entire platform.", "path": ["client>gw", "gw>keys", "gw>rl", "gw>upstream", "authoring>validate", "validate>store", "store>dist", "dist>gw"], "kind": "result"}]}
...
:::viz architecture {"title": "Metrics platform: ingest through a buffer, query through a planner", "nodes": [{"id": "agents", "label": "Agents", "kind": "client", "col": 0, "row": 1, "note": "on every host"}, {"id": "ingest", "label": "Ingest gateway", "kind": "edge", "col": 1, "row": 1, "note": "always accepts"}, {"id": "buffer", "label": "Buffer", "kind": "queue", "col": 2, "row": 1, "note": "log or queue"}, {"id": "writers", "label": "Writers", "kind": "worker", "col": 3, "row": 1, "note": "batch, compress"}, {"id": "storage", "label": "TSDB storage", "kind": "db", "col": 4, "row": 1, "note": "hot local, cold object"}, {"id": "notify", "label": "Notification system", "kind": "external", "col": 1, "row": 0}, {"id": "alert", "label": "Alert evaluator", "kind": "worker", "col": 2, "row": 0, "note": "runs rules on a schedule"}, {"id": "qapi", "label": "Query API + planner", "kind": "service", "col": 3, "row": 0}, {"id": "cache", "label": "Query cache", "kind": "cache", "col": 4, "row": 0, "note": "recent results"}], "edges": [{"from": "agents", "to": "ingest", "label": "scrape or push"}, {"from": "ingest", "to": "buffer"}, {"from": "buffer", "to": "writers"}, {"from": "writers", "to": "storage"}, {"from": "qapi", "to": "cache"}, {"from": "qapi", "to": "storage"}, {"from": "alert", "to": "qapi"}, {"from": "alert", "to": "notify", "label": "fire"}], "steps": [{"title": "Samples arrive", "explain": "Agents send samples to the ingest gateway, which validates them and appends to the buffer. It answers fast and never waits on storage.", "interview": "Say that ingestion must keep accepting even when storage is degraded, because that is exactly when the data is most needed.", "path": ["agents>ingest", "ingest>buffer"], "kind": "invariant"}, {"title": "Writers drain the buffer", "explain": "Writers read batches from the buffer, compress them, and write to storage. If storage is slow, the buffer grows into a backlog instead of dropping data.", "interview": "Say that the buffer decouples the arrival rate from the write rate, and name how you notice trouble: the buffer's lag, not a failed request.", "path": ["buffer>writers", "writers>storage"], "kind": "decision", "hot": ["storage"]}, {"title": "A query is planned", "explain": "The query API parses the expression, works out which series and time range it needs, checks the cache, and reads only those blocks from storage.", "interview": "Say that most dashboard queries repeat every few seconds, so caching the recent window removes most of the storage reads.", "path": ["qapi>cache", "qapi>storage"], "kind": "tradeoff"}, {"title": "Alerts use the same query path", "explain": "The alert evaluator runs each rule as a query on a schedule. When a condition holds for long enough, it hands off to the notification system.", "interview": "Say that alerts reuse the query API so a rule and a dashboard cannot disagree, and that the evaluator must be the most available part of the platform.", "path": ["alert>qapi", "alert>notify"], "kind": "invariant"}, {"title": "The whole picture", "explain": "Agents, ingest, buffer, writers, storage on the write side; planner, cache and storage on the read side; alerts on top of the read side.", "interview": "Close by naming the defining constraint: cardinality. One high-cardinality label multiplies the series count and takes the platform down.", "path": ["agents>ingest", "ingest>buffer", "buffer>writers", "writers>storage", "qapi>cache", "qapi>storage", "alert>qapi", "alert>notify"], "kind": "result"}]}
...
def case_study_topic() -> dict:
    ...
    return _sd_topic(
        ...
        "Twenty-two complete interview walkthroughs: requirements, estimates, APIs, data models, a first design, the bottleneck, and the design evolving under interviewer pressure.",
        "HARD",
        27,
        [
            _url_shortener(),
